...
function renderMisGuifos(arrayGifId) {
  ...
  arrayGifId.forEach((element) => {
    ...
    if (
      element.data.images.fixed_width_downsampled.width /
        element.data.images.fixed_width_downsampled.height >
      1.59
    ) {
...
function saveGifLocalStorage(gifID) {
  /* creo un array de ids de mis guifos */
...
function styleUploading() {
  document.querySelector(".box-title-container h2").innerHTML =
    "Subiendo guifo";
  ...
  document
    .getElementById("abortPostButton")
    .addEventListener("click", abortPost, false);
}
...
function styleUploaded() {
  ...
  document
    ...
    .addEventListener("click", downloadGuifo, false);
  document
    .getElementById("copylink")
    .addEventListener("click", copylinkGuifo, false);
...
function postGif() {
  ...
  fetch(
    "https://upload.giphy.com/v1/gifs?api_key=" +
      giphyKey +
      "&source_image_url=" +
      blobURL,
    {
...
function styleStopRecord() {
  ...
  if (!document.querySelector("#recordedvideo")) {
    //me fijo si ya está creado (ej. caso REPEAT)
...
function styleVideoStream() {
  ...
  document.getElementsByClassName("window-img")[0].classList.add("noshow");

  document.getElementsByClassName("instrucciones")[0].classList.add("noshow");

  document.querySelector(".twobuttons").classList.add("noshow");
...
document.getElementsByClassName("arrowimg")[0].addEventListener(
  "click",
  function () {
...
document.getElementById("listo").addEventListener(
  "click",
  function () {
    location.reload();
  },
